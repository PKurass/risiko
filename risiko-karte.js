/* =====================================================================
   ...
   abgetastet (über die browsereigene Kurvenmathematik), vereinfacht und
   dauerhaft gespeichert. Danach ist kein Auswählen und kein Internet
   mehr nötig.
   Schnittstelle (wie zuvor WorldMapGeo):
     SvgMap.load()        → Promise<boolean>  true = aus Cache geladen
     SvgMap.importSvg(txt)→ SVG-Text einlesen, cachen, bereitstellen
     SvgMap.polys/center/color/W/H
   ===================================================================== */
...
function importSvg(text,feinheit){
  ...
  // SVG unsichtbar in die Seite hängen, damit getPointAtLength & Farben funktionieren
...
/* einfache CSS.escape-Ersatz für ältere Browser */
...
function load(){
  return new Promise(res=>{
    // 1. Fest integrierte Datei (risiko-daten.js) hat Vorrang
    if(window.RISIKO_MAP&&window.RISIKO_MAP.polys){useData(window.RISIKO_MAP);return res(true);}
    // 2. Sonst der gespeicherte Cache im Browser
    try{
